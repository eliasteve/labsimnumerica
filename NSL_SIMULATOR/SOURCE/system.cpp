...
void System :: move(int i){ // Propose a MC move for particle i
  if(_sim_type == 3){ //Gibbs sampler for Ising
    double pPlus = 1/(1+exp( -2*_beta*(_J*(_particle(this->pbc(i-1)).getspin() + _particle(this->pbc(i+1)).getspin()) + _H) ));
    if(_rnd.Rannyu() < pPlus) _particle(i).setspin(1);
    else _particle(i).setspin(-1);
    // Gibbs had acceptance prob. 1
    _naccepted++;
  } else {           // M(RT)^2
...
void System :: initialize_properties(){ // Initialize data members used for measurement of properties
  ...
  if (input.is_open()){
    ...
    this->check_property_dependences();
...
void System :: measure(){ // Measure properties
  ...
  double magnetization_temp=0.0;
  double virial=0.0;
  if (_measure_penergy or _measure_pressure or _measure_gofr) {
    //std::cout << "in GOFR cycle..." << std::endl;
    for (int i=0; i<_npart-1; i++){
      for (int j=i+1; j<_npart; j++){
        ...
        //std::cout << "Particle pair (" << i+1 << ", " << j+1 << ")" << std::endl;
        // Writing for GOFR measurement
        if(_measure_gofr && dr < _halfside.min()) {//Avoid trying to write past the last position of the array
          index = floor(dr/_bin_size);
          _measurement(_index_gofr + index) += 2; //The cycle runs over half the particle pairs, the factor of 2 compensates
        }
        // Writing for pressure, energy measurements
        if(dr < _r_cut){
          if(_measure_penergy)  penergy_temp += 1.0/pow(dr,12) - 1.0/pow(dr,6); // POTENTIAL ENERGY
          // Technically the virial has a factor of 48/3 in front of
          // it, but I don't want to do that multiplication once per particle,
          // I'll do it just once later.
          if (_measure_pressure)  virial += 1./pow(dr, 12) - 0.5/pow(dr, 6);// PRESSURE
        }
      }
    }
    //Normalize histogram
  ...
  if (_measure_tenergy){
    if (_sim_type < 2) _measurement(_index_tenergy) = kenergy_temp + penergy_temp;
      ...
      _measurement(_index_tenergy) = tenergy_temp/double(_npart);
    }
  }
  // TEMPERATURE ///////////////////////////////////////////////////////////////
  ...
  if (_measure_magnet or _measure_chi) {
    for (int i=0; i<_npart; i++) {
      magnetization_temp += _particle(i).getspin();
    }
    if (_measure_magnet) _measurement(_index_magnet) = magnetization_temp/_npart;
  }
  // SPECIFIC HEAT /////////////////////////////////////////////////////////////
  if (_measure_cv) {
  ...
  // SUSCEPTIBILITY ////////////////////////////////////////////////////////////
  if (_measure_chi) {
    _measurement(_index_chi) = pow(magnetization_temp, 2);
  }
...
void System :: averages(int blk){
  ...
  ofstream coutf;
  coutf << std::scientific;// << std::setprecision(16);
  double average, sum_average, sum_ave2;

  std::string gofr_filename;

  _average     = _block_av / double(_nsteps);
  if (_measure_cv) {
  // _block_av(_index_cv) curently contains a measure of H^2, replace it
  // with the actual cv. Also remember the value of the total energy is 
  // _per particle_, while you need the _total_ energy.
    _average(_index_cv) = (pow(_beta, 2) * ( _average(_index_cv) - pow(_average(_index_tenergy)*_npart, 2) ))/_npart;
  }
  if (_measure_chi) {
    _average(_index_chi) *= _beta/_npart;
  }
  _global_av  += _average;
  ...
  if (_measure_penergy){
    ...
    coutf << setw(14) << blk 
          << setw(14) << average
          << setw(14) << sum_average/double(blk)
          << setw(14) << this->error(sum_average, sum_ave2, blk) << endl;
    coutf.close();
  }
  // KINETIC ENERGY ////////////////////////////////////////////////////////////
  if (_measure_kenergy){
  ...
  // GOFR //////////////////////////////////////////////////////////////////////
  if (_measure_gofr){
    gofr_filename = blk == _nblocks ? "../OUTPUT/gofr.dat" : "../OUTPUT/gofr_blk" + std::to_string(blk) + ".dat";
    coutf.open(gofr_filename);
    coutf << "# DISTANCE:     AVE_GOFR:        ERROR:" << std::endl;
    //cout << "Opened " << gofr_filename << std::endl;
    for (int i = 0; i < _n_bins; i++) {
      //std::cout << i << std::endl;
      average  = _average(_index_gofr+i);
      sum_average = _global_av(_index_gofr+i);
      sum_ave2 = _global_av2(_index_gofr+i);
      coutf << setw(14) << (i+0.5)*_bin_size //Pick midpoint of the bin as representative value for distance
            << setw(14) << sum_average/double(blk)
            << setw(14) << this->error(sum_average, sum_ave2, blk) << endl;
    }
    coutf.close();
  ...
  coutf << setw(14) << blk << setw(14) << fraction << endl;
  coutf.close();
...
int System :: get_neqsteps() {
  return _neqsteps;
}

//Checks wether we try to measure some property *and* not measure some
//other property upon which the first depends (e.g. how temperature
//depends on kinetic energy)
void System :: check_property_dependences() {
  ...
  if (_measure_cv and !_measure_tenergy) {
    std::cerr << "Warning: attempting to measure the heat capacity without measuring the total energy. The simulation still runs but the measurement of the heat capacity has undefined behaviour." << std::endl;
    coutf << "Warning: attempting to measure the heat capacity without measuring the total energy. The simulation still runs but the measurement of the heat capacity has undefined behaviour." << std::endl;
  }

  if (_measure_temp and !_measure_kenergy) {
    std::cerr << "Warning: attempting to measure the temperature without measuring the kinetic energy. The simulation still runs but the measurement of the temperature has undefined behaviour." << std::endl;
    coutf << "Warning: attempting to measure the temperature without measuring the kinetic energy. The simulation still runs but the measurement of the temperature has undefined behaviour." << std::endl;
  }

  if (_measure_pressure and !_measure_temp) {
    std::cerr << "Warning: attempting to measure the pressure without measuring the temperature. The simulation still runs but the measurement of the pressure has undefined behaviour." << std::endl;
    coutf << "Warning: attempting to measure the pressure without measuring the temperature. The simulation still runs but the measurement of the pressure has undefined behaviour." << std::endl;
  }

  if(_measure_chi and !_measure_magnet) {
    std::cerr << "Warning: attempting to measure the susceptibility without measuring the magnetization. The simulation still runs but the measurement of the susceptibility has undefined behaviour." << std::endl;
    coutf << "Warning: attempting to measure the susceptibility without measuring the magnetization. The simulation still runs but the measurement of the susceptibility has undefined behaviour." << std::endl;
  }
